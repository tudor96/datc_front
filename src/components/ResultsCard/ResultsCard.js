                ...
                <GridItem xs={12} sm={12} md={8}>
                    <Card>
                        <CardHeader color="primary">
                            <h4 className={classes.cardTitleWhite}>Rezultate Eveniment</h4>
                        </CardHeader>
                        <CardBody>
                            <GridContainer>
                                {props.pollResults.map(question => (
                                    <GridItem xs={12} sm={12} md={5}>
                                        <h3 className={classes.cardCategory}>{question[0].questionName}</h3>
                                        {question.map(option => (
                                            <h6 className={classes.cardTitle}>OPTIUNEA: <b>{option.name}</b> CU NUMARUL DE VOTURI: <b>{option.votes} </b></h6>
                                        ))}
                                    </GridItem>
                                ))}
