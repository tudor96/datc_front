...
export default function PrezidentialCard(props) {
    ...
    function saveQuestionHandler() {
        let question = { "name": "Prezidentiale", "options": [] }
        let allOptions = [];
        options.map((option, index) => {
            allOptions.push({ "name": document.getElementById("option" + option.id).value, "description": document.getElementById("description" + option.id).value, "idPartid": partid[index].idPartid });
        })
                    ...
                    <GridContainer>
                        ...
                        <GridItem xs={12} sm={12} md={6}>
                            <FormControl className={classes.formControl}>
                                ...
                                <h3 id={"h4" + option.id} >{partid[index].name === null ? "Partid" : partid[index].name}</h3>
                            </FormControl>
                        </GridItem>
                        <GridItem xs={12} sm={12} md={6}>
                            <CustomInput
                                labelText="Scurta descriere"
